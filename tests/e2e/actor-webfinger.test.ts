...
describe('actor endpoint', () => {
  it('returns ActivityStreams actor JSON with correct Content-Type', async () => {
    ...
    expect(actor.inbox).toBe(`${devServerUrl}/actor/inbox`)
    expect(actor.outbox).toBe(`${devServerUrl}/actor/outbox`)
    expect(actor.followers).toBe(`${devServerUrl}/actor/followers`)
    expect(actor.following).toBe(`${devServerUrl}/actor/following`)
    expect(actor.liked).toBe(`${devServerUrl}/actor/liked`)
