...
vi.mock('../../src/services/serializeFollowingCollection.js', () => ({
  serializeFollowingCollection: mockSerializeCollection
}))

vi.mock('../../src/services/serializeFollowingPage.js', () => ({
  serializeFollowingPage: mockSerializePage
}))
...
function makeContext(overrides: Partial<Context> = {}): Context {
  return {
    ...
    account: {},
    params: { actor: 'actor' },
    url: new URL('http://localhost/actor/following'),
    next: vi.fn(),
    ...overrides
  } as Context
}
...
describe('actor-router following GET AS2 JSON', () => {
  it('returns OrderedCollection JSON when Accept is application/activity+json (no auth required)', async () => {
    const jsonBody = {
      '@context': 'https://www.w3.org/ns/activitystreams',
      id: 'http://localhost:9999/actor/following',
      type: 'OrderedCollection',
      totalItems: 1,
      first: 'http://localhost:9999/actor/following/pages/1'
    }
    ...
    const req = new Request('http://localhost/actor/following', {
      method: 'GET',
    ...
    expect(body.id).toBe('http://localhost:9999/actor/following')
  ...
  it('returns OrderedCollectionPage JSON for a page path with AS2 Accept', async () => {
    const jsonBody = {
      '@context': 'https://www.w3.org/ns/activitystreams',
      id: 'http://localhost:9999/actor/following/pages/1',
      type: 'OrderedCollectionPage',
      partOf: 'http://localhost:9999/actor/following',
    ...
    const { default: handler } = await import('../../netlify/functions/actor-router.mts')
    const req = new Request('http://localhost/actor/following/pages/1', {
      method: 'GET',
      headers: { accept: 'application/activity+json' }
    })
    ...
    const ctx = makeContext({
      ...
      url: new URL('http://localhost/actor/following/pages/1')
    })
  ...
  it('proxies Turtle when Accept is text/turtle (no auth required)', async () => {
    ...
    const innerFetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      text: () => Promise.resolve('<http://localhost:9998/actor/following/> a <https://www.w3.org/ns/activitystreams#OrderedCollection> .'),
  ...
  it('proxies Turtle when Accept is absent (back-compat, no auth required)', async () => {
    ...
    const { default: handler } = await import('../../netlify/functions/actor-router.mts')
    const req = new Request('http://localhost/actor/following', { method: 'GET' })
  ...
  it('OPTIONS preflight returns 204 with CORS headers', async () => {
    const { default: handler } = await import('../../netlify/functions/actor-router.mts')
    const req = new Request('http://localhost/actor/following', { method: 'OPTIONS' })
